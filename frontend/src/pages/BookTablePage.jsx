...
const BookTablePage = () => {
  ...
  const showSuccessAndFinish = (details) => {
  ...
  const handleAdminBook = async () => {
    ...
    try {
      ...
      if (data.tableNumber) {
        toast.success(
          `Admin Booking Confirmed! Assigned Table: ${data.tableNumber}`,
        );
      } else {
      ...
      setError(
        err.response?.data?.message || err.message || "Admin booking failed",
      );
      toast.error(
        err.response?.data?.message || err.message || "Admin booking failed",
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!availData.available) {
        throw new Error(
          availData.message || "No tables available for this slot.",
        );
      }
      ...
      if (paymentMethod === "wallet") {
        if (Number(user.walletBalance || 0) < paymentAmount) {
          setError(
            `Insufficient wallet balance. Required: ₹${paymentAmount}. Current: ₹${Number(user.walletBalance || 0)}.`,
          );
        ...
        console.log(
          "Wallet Payment verified and booking created:",
          walletData.booking,
        );
        if (walletData.booking?.tableNumber) {
          toast.success(
            `Booking Confirmed! Assigned Table: ${walletData.booking.tableNumber}`,
          );
        } else {
        ...
        handler: async (response) => {
          ...
          try {
            ...
            if (result.booking.tableNumber) {
              toast.success(
                `Booking Confirmed! Assigned Table: ${result.booking.tableNumber}`,
              );
            } else {
              toast.success("Booking Confirmed! (Table pending assignment)");
            }
      ...
      rzp.on("payment.failed", function (response) {
        ...
        if (
          ...
          response.error.description.includes("cancel")
        ) {
      ...
      <div className="book-table-box">
          ...
          <div className="premium-label-wrapper">
            <span className="logo-small">
              <span className="icon-box">
                <Icons.utensils size={18} className="lucide" />
              </span>{" "}
              SMARTDINE
            </span>
          </div>
        ...
        {error && (
          <div className="error-msg-banner">
            <span className="icon-box">
              <Icons.alertCircle size={20} className="lucide" />
            </span>
            <span>{error}</span>
          </div>
        )}
        ...
        {submitted ? (
          <div className="booking-success-card">
            ...
            <div className="success-details">
              <div className="success-detail-row">
                ...
                <span className="success-detail-value">
                  {bookingDetails?.tableNumber || "Pending"}
                </span>
              </div>
              <div className="success-detail-row">
              ...
              <div className="success-detail-row">
                ...
                <span className="success-detail-value">
                  {bookingDetails?.guests || formData.guests}
                </span>
              </div>
            </div>
            ...
            <button onClick={() => navigate("/")} className="pf-primary-btn">
              Go Home
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="booking-form">
            ...
            <div className="user-info-pill">
              <div className="user-info-col">
                <span className="icon-box">
                  <Icons.user size={16} className="lucide info-icon" />
                </span>
              ...
              <div className="user-info-col">
                <span className="icon-box">
                  <Icons.mail size={16} className="lucide info-icon" />
                </span>
              ...
              <div className="user-info-col">
                <span className="icon-box">
                  <Icons.phone size={16} className="lucide info-icon" />
                </span>
                <span className="info-label">Phone:</span>
                <span className="info-value">
                  {user?.phone || "9823743793"}
                </span>
              </div>
            </div>
            ...
            <div className="booking-card">
              <div className="booking-row-horizontal">
                <div className="booking-field-compact">
                  <label>
                    <span className="icon-box">
                      <Icons.calendar size={14} className="lucide" />
                    </span>{" "}
                    DATE
                  </label>
                  <BookingCalendar
                ...
                <div className="booking-field-compact time-field-auto">
                  <label>
                    <span className="icon-box">
                      <Icons.clock size={14} className="lucide" />
                    </span>{" "}
                    TIME
                  </label>
                  {isLoadingSlots ? (
                ...
                <div className="booking-field-compact">
                  <label>
                    <span className="icon-box">
                      <Icons.user size={14} className="lucide" />
                    </span>{" "}
                    GUESTS
                  </label>
                  {isLoadingSlots ? (
              ...
              <div className="payment-methods">
                ...
                >
                  <div className="payment-card-icon">
                    <span className="icon-box">
                      <Icons.card size={24} className="lucide" />
                    </span>
                  </div>
                ...
                >
                  <div className="payment-card-icon">
                    <span className="icon-box">
                      <Icons.wallet size={24} className="lucide" />
                    </span>
                  </div>
                  <div className="payment-card-info">
            ...
            <div className="booking-actions-group">
              <button
                type="submit"
                className={`reserve-btn ${loading ? "loading" : ""}`}
                disabled={loading}
              >
                {loading ? (
                  <>
              ...
              {isAdmin && (
                <button
                  ...
                  className="submit-btn admin-book-btn"
                >
