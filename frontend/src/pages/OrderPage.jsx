        ...
        <div className="menu-skeleton-grid">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="menu-skeleton-card">
              <div
                className="skeleton"
                style={{ height: 64, borderRadius: 10, marginBottom: 10 }}
              />
...
const OrderPage = () => {
      ...
      if (user && token) {
        try {
          ...
          const active = bookings.find(
            (b) => b.status === "confirmed" && b.tableNumber,
          );
          if (active) {
    ...
    try {
      ...
      if (paymentMethod === "wallet") {
        if (Number(user.walletBalance || 0) < totalAmountFloat) {
          setError(
            `Insufficient wallet balance. Required: ₹${totalAmountFloat}.`,
          );
        ...
        toast.success(
          `Payment successful! Order placed. Total: ₹${totalAmountFloat}`,
        );
        setCart([]);
        setIsCartOpen(false);
        navigate("/");
        ...
        modal: {
          ondismiss: () => {
            ...
            toast("Payment cancelled.", {
              icon: <Icons.ban size={20} className="icon-muted" />,
            });
          },
        },
      };
  ...
  const groupedMenu = useMemo(() => {
    ...
    groups["Favorites"] = availableItems.filter(
      (item) =>
    ...
    <div className="order-container">
      ...
      <div className={`order-layout ${isCartOpen ? "cart-open" : ""}`}>
        <aside className="categories-sidebar">
          <h3>
            <Icons.folderOpen size={20} className="inline-icon" /> Categories
          </h3>
        ...
        <div className="menu-content-area">
          <div className="menu-top-row">
            ...
            {cart.length > 0 && !isCartOpen && (
              <button
                className="top-view-cart-btn"
                onClick={() => setIsCartOpen(true)}
              >
                <Icons.shoppingBag size={18} />
                <span>
                  View Cart ({cart.reduce((a, b) => a + Number(b.quantity), 0)})
                </span>
              </button>
            )}
          </div>
          ...
          {loading ? (
            ...
            <div className="menu-display" style={{ scrollBehavior: "smooth" }}>
                            ...
                            <MenuCard
                              ...
                              item={{
                                ...
                                name: debouncedSearch
                                  ...
                                  : item.name,
                                description:
                                  debouncedSearch && item.description
                                    ? highlightText(
                                        item.description,
                                        debouncedSearch,
                                      )
              ...
              {availableItems.length > 0 &&
                categoriesList.every((c) => {
                  const items =
                    c === "All" ? availableItems : groupedMenu[c] || [];
                  return (
                    items.filter(
                      (i) =>
                        i.name
        ...
        {/* Backdrop removed're moving from overlay to grid-integrated sidebar */}
        <div className={`cart-section ${isCartOpen ? "open" : ""}`}>
          ...
          <div className="table-status-banner compact-banner">
            {assignedTable ? (
              <p>
                <Icons.utensils size={18} className="inline-icon" /> Table{" "}
                {assignedTable} (Dine-In)
              </p>
            ) : (
              <p>
                <Icons.shoppingBag size={18} className="inline-icon" /> Parcel
                (Table Pending)
              </p>
            )}
          </div>
