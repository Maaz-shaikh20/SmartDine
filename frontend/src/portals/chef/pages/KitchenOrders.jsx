...
import { Clock, Utensils, User, FolderOpen } from "lucide-react";
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const hasNewOrder = newOrders.some(
        (o) =>
          o.status === "pending" &&
          !prevOrdersRef.current.find((prev) => prev.id === o.id),
      );

      if (hasNewOrder && prevOrdersRef.current.length > 0) {
        soundRef.current?.play().catch(() => {});
        toast("New Order Received!", {
          icon: <Icons.bell size={20} className="icon-primary" />,
        });
      }
    ...
    <div className="chef-page">
        ...
        >
          <div>
            ...
            <p className="admin-page-subtitle">
              Live kitchen management and order processing.
            </p>
            <div className="admin-header-divider"></div>
          </div>
          <div className="kitchen-legend">
            <div className="legend-item">
              <span className="dot normal"></span> 0-10m
            </div>
            <div className="legend-item">
              <span className="dot warning"></span> 10-20m
            </div>
            <div className="legend-item">
              <span className="dot urgent"></span> 20m+
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="admin-tabs" style={{ marginBottom: "32px" }}>
        ...
        >
          ...
          Kitchen Orders
          <span
            className="chef-tab-count"
            style={{
              marginLeft: "8px",
              background:
                activeTab === "active" ? "white" : "var(--brand-primary)",
              color: activeTab === "active" ? "var(--brand-primary)" : "white",
            }}
          >
            {orders.length}
          </span>
        ...
        >
          ...
          <span
            ...
            style={{
              marginLeft: "8px",
              background:
                activeTab === "history" ? "white" : "var(--brand-primary)",
              color: activeTab === "history" ? "var(--brand-primary)" : "white",
            }}
          >
            {history.length}
          </span>
      ...
      {activeTab === "active" ? (
        orders.length === 0 ? (
          ...
          >
            <div className="chef-empty-icon" style={{ opacity: 0.3 }}>
              <Icons.utensils size={64} />
            </div>
            <h3 className="chef-empty-title" style={{ marginTop: "20px" }}>
              No Active Orders
            </h3>
            <p className="chef-empty-sub">
              The kitchen is all caught up. New orders will appear here
              automatically.
            </p>
          </div>
        ) : (
          <div className="chef-cards-grid">
            {orders.map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className={`admin-card ${getUrgencyClass(order.createdAt)}`}
                ...
                >
                  ...
                  <div
                    className="chef-items-container"
        ...
        <div className="chef-empty-state">
          <div className="chef-empty-icon">
            <FolderOpen size={48} />
          </div>
          <h3 className="chef-empty-title">History is Empty</h3>
          <p className="chef-empty-sub">
            Completed orders from today will show up here.
          </p>
        </div>
      ) : (
        <div className="chef-cards-grid">
            ...
            >
              ...
              <div className="chef-card-details">
                <div className="chef-detail-item">
                  <span className="chef-detail-icon">
                    <Utensils size={16} />
                  </span>
