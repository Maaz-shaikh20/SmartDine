...
const Bookings = ({ hideHeader = false }) => {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      await api.put(`/admin/bookings/${selectedBookingId}/cancel`, {
        reason: finalReason,
      });
  ...
  const updateTableAPI = async (id, tableId) => {
    try {
  ...
  const handleAssignFromModal = async (tableId) => {
    if (!selectedBookingId) return;
  ...
  const columns = [
      ...
      render: (booking) =>
        booking.tableId || booking.tableNumber ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            >
              Table {booking.tableId || booking.tableNumber}
            </span>
            <div style={{ display: "flex", gap: "6px" }}>
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  updateTableAPI(booking.id, null);
                }}
                style={{
                  padding: "4px 8px",
                  fontSize: "0.7rem",
    ...
    },
    {
      ...
      render: (booking) => (
        <span
          className={`status-pill-modern status-modern-${booking.status?.toLowerCase()}`}
        >
          {booking.status}
        </span>
      ),
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    const matchesStatus =
      !activeFilters.status || b.status === activeFilters.status;
    const matchesGuests =
      !activeFilters.guests ||
      (activeFilters.guests === "small"
        ? b.guests <= 2
        : activeFilters.guests === "medium"
          ? b.guests > 2 && b.guests <= 4
          : b.guests > 4);

    const hour = parseInt(b.time.split(":")[0]);
    const matchesTime =
      !activeFilters.timeSlot ||
      (activeFilters.timeSlot === "morning"
        ? hour < 12
        : activeFilters.timeSlot === "afternoon"
          ? hour >= 12 && hour < 17
          : hour >= 17);
    ...
    <div className={hideHeader ? "" : "management-page"}>
      {!hideHeader && (
        <header className="admin-page-header">
          ...
          <p className="admin-page-subtitle">
            Track and manage upcoming restaurant reservations.
          </p>
      ...
      {loading ? (
        ...
        <div className="error-state">
          <p>
            <Icons.alertCircle size={16} className="inline-icon" /> {error}
          </p>
      ...
      >
        <div style={{ textAlign: "center", marginBottom: "24px" }}>
          ...
          >
            ...
            release the assigned table and notify the customer.
          </p>
        </div>
