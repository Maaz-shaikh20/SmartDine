...
const Users = () => {
  ...
  const currentUser = JSON.parse(
    localStorage.getItem("smartdine_user") || "{}",
  );
  ...
  const getRoleBadgeClass = (role) => {
    switch (role.toLowerCase()) {
      case "admin":
        return "status-modern-cancelled";
      case "chef":
        return "status-modern-pending";
      default:
        return "status-modern-confirmed";
    }
  };
    ...
    { header: "Email", key: "email" },
    {
      ...
      render: (user) => (
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <span
            className={`status-pill-modern ${getRoleBadgeClass(user.role)}`}
