...
const AdminMenu = () => {
  ...
  const toggleAvailability = async (item) => {
    ...
    try {
      ...
      if (res.data) {
        setMenuItems((prev) =>
          prev.map((m) => (m.id === item.id ? { ...m, ...res.data } : m)),
        );
        toast.success(
          `${item.name} is now ${newStatus === "available" ? "visible" : "hidden"}`,
        );
      }
    } catch (err) {
  ...
  const handleSaveDish = async () => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        setMenuItems((prev) =>
          prev.map((m) =>
            m.id === editingItem.id ? { ...m, ...res.data } : m,
          ),
        );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item) => (
        <span
          className={`status-pill-modern ${item.status === "available" ? "status-modern-confirmed" : "status-modern-cancelled"}`}
        >
    ...
    <div className="management-page">
        ...
        >
          <div>
            ...
            <p className="admin-page-subtitle">
              Add, edit, and control visibility of your restaurant's dishes.
            </p>
          </div>
          <Button
            variant="primary"
      ...
      {loading ? (
        ...
        <div className="error-state">
          <p>⚠️ {error}</p>
          <Button variant="primary" onClick={fetchMenu}>
            Retry
          </Button>
        </div>
      ) : (
          ...
          >
            <div>
              ...
              <span style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                Dish will appear on current menu
              </span>
            </div>
            <div
