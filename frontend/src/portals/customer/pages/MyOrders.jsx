...
const MyOrders = () => {
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      const [bookingsRes, ordersRes, upcomingRes, reviewsRes] =
        await Promise.all([
        ...
        toast(
          () => (
            ...
            >
              ...
              Reminder: You have a booking at{" "}
              {formatTime(upcomingData.upcomingBooking.time)} today!
            </span>
          ),
      ...
      if (Array.isArray(rawOrders)) {
        processedOrders = rawOrders.map((o) => {
          let items = o.items;
          try {
          ...
          return {
            ...
            review: reviewsData.find((r) => r.orderId === o.id),
          };
        });
      }
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      if (data.walletBalance !== undefined && user) {
        ...
        toast.success(
          data.message || "Booking cancelled. Amount refunded to Wallet.",
        );
      } else {
  ...
  if (error)
    return (
      <div className="cp-error">
        ...
        <p>{error}</p>
        <button onClick={fetchUserData} className="cp-retry-btn">
          Retry
        </button>
      </div>
    );
  ...
  if (!user)
    return (
      <div className="cp-loading">
        ...
        <button onClick={() => navigate("/login")} className="cp-retry-btn">
          Go to Login
        </button>
      </div>
    );
    ...
    <div className="cp-page">
      <div className="cp-content">
        ...
        <div className="cp-welcome fade-up">
          <div>
            ...
            <p className="cp-subtitle">
              Welcome back, <strong>{user.name}</strong>! Here's everything in
              one place.
            </p>
          </div>
                    ...
                    {isCancellable(booking) && (
                      ...
                      >
                        {cancellingId === booking.id ? (
                          <>
                            <Icons.loader size={14} className="inline-icon" />{" "}
                            Cancelling…
                          </>
                        ) : (
                          <>
                            <Icons.close size={14} className="inline-icon" />{" "}
                            Cancel Booking
                          </>
                        )}
                      </button>
                  ...
                  <div key={order.id} className="cp-card">
                    ...
                    {order.status?.toLowerCase() === "pending" ? (
                      <button
                        ...
                        style={{ marginTop: 14 }}
                      >
                        <Icons.close size={14} className="inline-icon" /> Cancel
                        Order
                      </button>
                    ) : order.status?.toLowerCase() === "preparing" ||
                      order.status?.toLowerCase() === "ready" ? (
                      <div
                        className="cp-cancel-blocked"
                        style={{
                          marginTop: 14,
        ...
        <Modal
          ...
          title={`Rate Order #${String(reviewOrder?.id || "")
            .slice(-6)
            .toUpperCase()}`}
          size="md"
        >
          <div
            className="cp-star-rating"
            style={{
              display: "flex",
              justifyContent: "center",
              gap: "10px",
            ...
            {[1, 2, 3, 4, 5].map((s) => (
              ...
              >
                <Icons.star
                  ...
                  color={
                    s <= rating ? "var(--brand-primary)" : "var(--text-dim)"
                  }
        ...
        >
          ...
          >
            If you cancel this order, the full amount will be credited to your
            SmartDine Wallet.
          </p>
          <div
            className="modal-actions"
        ...
        >
          <div style={{ marginBottom: "24px" }}>
            ...
            >
              <Icons.alertCircle size={14} className="inline-icon" /> You can
              cancel only within 5 minutes of booking. The fee will be credited
              to your Wallet.
            </div>
          </div>
          <div
